// this is file for the face recognition, you should open it with arduino IDE.
...
const char* ssid = "k";  // Ganti dengan SSID Wi-Fi Anda
const char* password = "kimshahaha";  // Ganti dengan password Wi-Fi Anda

String serverName = "192.168.249.63";  // Ganti dengan IP address server Anda
String serverPath = "/esp32cam_gallery/upload_img.php";     
const int serverPort = 80;

WiFiClient client;

// Konfigurasi pin kamera untuk AI Thinker
...
const int Interval = 15000;    // Interval pengambilan foto dalam milidetik
...
void setup() {
  ...
  // Inisialisasi kamera dengan spesifikasi tinggi jika PSRAM ditemukan
  if(psramFound()){
    config.frame_size = FRAMESIZE_VGA;
    config.jpeg_quality = 12;  // 0-63 semakin rendah angka semakin tinggi kualitas
    config.fb_count = 2;
  } else {
    config.frame_size = FRAMESIZE_SVGA;
    config.jpeg_quality = 8;  // 0-63 semakin rendah angka semakin tinggi kualitas
    config.fb_count = 1;
  }
...
void sendPhoto() {
  ...
  // Pre-capture untuk timing yang lebih akurat
  for (int i = 0; i <= 3; i++) {
    camera_fb_t * fb = NULL;
